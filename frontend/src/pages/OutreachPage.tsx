...
export function OutreachPage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      meetings: all.filter((o) => o.channel === "meeting").length,
    };
  }, [data]);
  ...
  return (
    <div className="p-5 sm:p-6">
      <PageHeader
        eyebrow="Engagement"
        title="Outreach"
        subtitle="Every touch to prospective partner communities."
        actions={
          canWrite && (
            <Button onClick={() => setLogOpen(true)}>
              <Icon name="plus" size={16} /> Log outreach
            </Button>
          )
        }
      />
      ...
      <div className="mt-4 card-base overflow-hidden">
        {isLoading ? (
          ...
          <ErrorState message="Could not load outreach." onRetry={() => refetch()} />
        ) : rows.length === 0 ? (
          <EmptyState icon={<Icon name="mail" />} title="No outreach logged" subtitle="Log the first touch to get started." />
        ) : (
          <div className="divide-y divide-line">
            {rows.map((o) => {
              ...
              return (
                <button
                  key={o.id}
                  onClick={() => o.lead_id && navigate(`/leads?focus=${o.lead_id}`)}
      ...
      <LogOutreachModal open={logOpen} onClose={() => setLogOpen(false)} />
    </div>
  );
}
...
function LogOutreachModal({ open, onClose }: { open: boolean; onClose: () => void }) {
